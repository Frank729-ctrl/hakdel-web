...
async function lookup(ip: string) {
  const res = await fetch('/api/tools/ip', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ip }) })
...
export default function IpPage() {
  return (
    <ToolShell title="IP Reputation" description="Check an IP against AbuseIPDB, VirusTotal, and Shodan."
      inputLabel="IP Address" placeholder="1.2.3.4" onSubmit={lookup}
      renderResult={(data: any) => (
        <div className="space-y-4">
          {data.abuseipdb?.data && (
            <div className="card p-5">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {[['Abuse Score', `${data.abuseipdb.data.abuseConfidenceScore}%`], ['Total Reports', data.abuseipdb.data.totalReports],
                  ['Country', data.abuseipdb.data.countryCode], ['ISP', data.abuseipdb.data.isp]].map(([k, v]) => (
              ...
              {data.abuseipdb.data.abuseConfidenceScore > 0 && (
                <div className="mt-3 flex items-center gap-2">
                  <div className="h-2 flex-1 bg-border-2 rounded-full overflow-hidden">
                    <div className="h-full bg-danger rounded-full" style={{ width: `${data.abuseipdb.data.abuseConfidenceScore}%` }} />
                  </div>
          ...
          {data.shodan?.ports && (
            <div className="card p-5">
              <div className="section-title">Shodan — Open Ports</div>
