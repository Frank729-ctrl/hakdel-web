...
export default function HashPage() {
  return (
    <ToolShell
      title="Hash Check"
      description="Look up a file hash against VirusTotal and MalwareBazaar."
      inputLabel="MD5 / SHA1 / SHA256 hash"
      placeholder="d8e8fca2dc0f896fd7cb4cb0031ba249"
      buttonLabel="Check Hash"
      onSubmit={lookup}
        ...
        <div className="space-y-4">
          {data.virustotal?.data && (
            <div className="card p-5">
              ...
              {data.virustotal.data.attributes?.popular_threat_classification && (
                <div className="text-sm text-dim">
                  Threat: <span className="text-danger font-medium">
                    {data.virustotal.data.attributes.popular_threat_classification.suggested_threat_label}
                  </span>
                </div>
              )}
            </div>
          ...
          {data.malwarebazaar?.data?.[0] && (
            <div className="card p-5">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {[
                  ['File Name', data.malwarebazaar.data[0].file_name],
                  ['MIME Type', data.malwarebazaar.data[0].file_type_mime],
                  ['Signature', data.malwarebazaar.data[0].signature],
                  ['First Seen', data.malwarebazaar.data[0].first_seen],
                ].map(([k, v]) => (
                  <div key={String(k)}>
