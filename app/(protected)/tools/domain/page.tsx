...
export default function DomainPage() {
  return (
    <ToolShell
      ...
      description="Enumerate subdomains via certificate transparency logs and DNS records."
      inputLabel="Domain"
      placeholder="example.com"
      buttonLabel="Enumerate"
      onSubmit={lookup}
        ...
        <div className="space-y-4">
            ...
            <div className="card p-5">
              ...
              <div className="grid grid-cols-2 gap-1 max-h-80 overflow-auto">
                {data.subdomains.map((s: string) => (
                  <div key={s} className="font-mono text-xs text-dim py-0.5">{s}</div>
                ))}
              </div>
            </div>
          )}
          {data.dns && (
            <div className="card p-5">
              ...
              <pre className="mono text-xs text-dim whitespace-pre-wrap bg-surface-2 p-4 rounded-lg overflow-auto max-h-64">
                {data.dns}
              </pre>
            </div>
          )}
