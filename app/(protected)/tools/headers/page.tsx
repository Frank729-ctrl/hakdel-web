...
export default function HeadersPage() {
  return (
    <ToolShell
      title="HTTP Headers"
      description="Analyze security headers for any URL."
      inputLabel="URL"
      inputType="url"
      ...
      renderResult={(data: any) => (
        <div className="space-y-4">
            ...
            <div className="space-y-2">
              {data.analysis?.map((h: any) => (
                <div key={h.header} className="flex items-start gap-3 py-2 border-b border-border last:border-0">
                  ...
                  <span className={h.present ? 'badge-green' : 'badge-red'}>
          ...
          <div className="card p-5">
            <div className="section-title">All Response Headers</div>
            <div className="space-y-1">
              {Object.entries(data.headers ?? {}).map(([k, v]) => (
                <div key={k} className="flex gap-3 text-xs">
                  <span className="font-mono text-accent w-48 flex-shrink-0 truncate">{k}</span>
                  <span className="text-dim truncate">{String(v)}</span>
                </div>
              ))}
            </div>
          </div>
