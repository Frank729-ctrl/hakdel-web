...
export default async function ScanHistoryPage() {
  ...
  return (
    <div>
      <Header
        title="Scan History"
        description={`${scans.length} scan${scans.length !== 1 ? 's' : ''} total.`}
        action={
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted mt-0.5 flex items-center gap-3">
                      ...
                      {scan.modules.length > 0 && (
                        <><span>·</span><span>{scan.modules.length} modules</span></>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className={`badge ${
                      scan.status === 'DONE' ? 'badge-green' :
                      scan.status === 'ERROR' ? 'badge-red' : 'badge-gray'
