...
interface Finding { severity: string; title: string; description: string; module: string }
interface ScanResult { score: number; grade: string; summary: string; findings: Finding[] }
...
export default function ScannerPage() {
  ...
  const toggleModule = (id: string) =>
    setModules((p) => p.includes(id) ? p.filter((m) => m !== id) : [...p, id])
  ...
  async function startScan(e: React.FormEvent) {
    e.preventDefault()
    setStatus('scanning'); setProgress(0); setResult(null); setError('')
    try {
      ...
      const poll = setInterval(async () => {
        const data = await fetch(`/api/scanner/status/${job_id}`).then((r) => r.json())
        ...
        if (data.status === 'done') {
          ...
          await fetch('/api/scanner/save', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ job_id, target_url: url, profile, modules: activeModules,
              score: data.result?.score, grade: data.result?.grade,
              summary: data.result?.summary, result: data.result }),
          })
        } else if (data.status === 'error') {
          clearInterval(poll); setError(data.message ?? 'Scan failed.'); setStatus('error')
        }
      }, 2000)
    } catch (err: any) {
  ...
  return (
    <div>
      <Header title="Security Scanner" description="Scan a target for vulnerabilities and misconfigurations."
        action={<Link href="/scanner/history" className="btn-secondary"><Clock size={14} /> History</Link>} />
        ...
        <div className="space-y-4">
          <form onSubmit={startScan} className="card p-5 space-y-4">
            <div>
              ...
              <input className="input font-mono" type="url" placeholder="https://example.com"
                value={url} onChange={(e) => setUrl(e.target.value)} required disabled={status === 'scanning'} />
            </div>
            <div>
              <label className="label">Scan Profile</label>
              <div className="grid grid-cols-3 gap-2">
                {PROFILES.map((p) => (
                  <button key={p.id} type="button" onClick={() => setProfile(p.id)} disabled={status === 'scanning'}
                    className={`p-3 rounded-lg border text-left transition-colors ${profile === p.id
                      ? 'bg-accent/10 border-accent/30 text-ink'
                      : 'bg-surface-2 border-border text-dim hover:text-ink'}`}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {MODULES.map((m) => (
                    <label key={m.id} className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" className="accent-accent" checked={modules.includes(m.id)}
                        onChange={() => toggleModule(m.id)} disabled={status === 'scanning'} />
          ...
          {status === 'scanning' && (
            <div className="card p-5">
              ...
              <div className="h-2 bg-border-2 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-accent to-accent-2 rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
              </div>
            </div>
          )}
          ...
          {status === 'error' && (
            <div className="card p-5 border-danger/20 bg-danger/5">
              <div className="flex items-center gap-2 text-danger mb-1"><AlertCircle size={16} /><span className="font-medium">Scan failed</span></div>
          ...
          {status === 'done' && result && (
            <div className="space-y-4">
              <div className="card p-5">
                <div className="flex items-center gap-6">
                  <div className="w-20 h-20 rounded-2xl bg-surface-2 flex flex-col items-center justify-center flex-shrink-0">
                    <div className={`text-3xl font-bold font-mono ${result.score >= 80 ? 'text-success' : result.score >= 60 ? 'text-warning' : 'text-danger'}`}>
                      {result.grade}
                    </div>
                    <div className="text-xs text-muted">{result.score}/100</div>
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-1"><CheckCircle size={14} className="text-success" /><span className="text-sm font-medium text-ink">Scan complete</span></div>
              ...
              {sortedFindings && sortedFindings.length > 0 && (
                <div className="card">
                  <div className="px-5 py-3 border-b border-border font-medium text-sm text-ink">{sortedFindings.length} Finding{sortedFindings.length !== 1 ? 's' : ''}</div>
        ...
        <div className="space-y-4">
          <div className="card p-5">
            ...
            <div className="space-y-3 text-sm text-dim">
              <p>Runs against the FastAPI scanner backend which performs real security checks.</p>
              <p>Results are saved to your scan history.</p>
            </div>
          </div>
          {!isPro && (
            <div className="card p-5 border-accent/20 bg-accent/5">
              <div className="text-sm font-medium text-ink mb-1">Free plan limit</div>
              <p className="text-xs text-dim mb-3">Free users can run 3 scans per day. Upgrade to Pro for unlimited scans.</p>
              <Link href="/upgrade" className="btn-primary w-full text-xs py-1.5">Upgrade to Pro →</Link>
            </div>
          )}
        </div>
      </div>
