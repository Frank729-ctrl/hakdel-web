        ...
        <div className="divide-y divide-border">
          {users.map((user, i) => {
            ...
            return (
              <div
                key={user.id}
                className={`px-5 py-3 grid grid-cols-[2rem_1fr_6rem_6rem_6rem_5rem] gap-4 items-center ${
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-ink truncate">
                      {user.username} {isMe && <span className="text-xs text-accent">(you)</span>}
                    </div>
                    {user.streakDays > 0 && (
                      <div className="text-xs text-warning">🔥 {user.streakDays}d streak</div>
                    )}
                  </div>
                </div>
                <div className="text-right font-mono text-sm text-ink">{user.xp.toLocaleString()}</div>
                <div className="text-right">
                  <span className="font-mono text-sm text-dim">Lv.{user.level}</span>
                </div>
                <div className="text-right font-mono text-sm text-muted">{user._count.scans}</div>
                <div className="text-right">
                  <span className={user.plan === 'PRO' ? 'badge-blue text-xs' : 'text-xs text-muted'}>{user.plan}</span>
                </div>
              </div>
            )
          })}
