...
import {
  ...
  Trophy, User, Settings, Bell, LogOut, ChevronRight,
...
const nav = [
  ...
  { label: 'Scanner', href: '/scanner', icon: Shield },
  {
    ...
    children: [
      ...
      { label: 'URL Check', href: '/tools/url' },
      { label: 'Email Check', href: '/tools/email' },
    ...
    <aside className="w-[220px] h-screen bg-surface border-r border-border flex flex-col fixed left-0 top-0 z-30 overflow-y-auto">
      ...
      <div className="flex items-center gap-2.5 px-4 py-4 border-b border-border">
        ...
        <div className="ml-auto">
          <span className={cn('badge text-[10px]', session?.user?.plan === 'PRO' ? 'badge-blue' : 'badge-gray')}>
            ...
            return (
              <div key={item.href}>
                <Link
                  ...
                  className={cn(
                    'nav-link',
                    (active || anyChildActive) && 'text-ink'
                  )}
                >
                  <Icon size={15} />
      ...
      <div className="px-2 py-3 border-t border-border space-y-0.5">
        ...
        <Link href="/notifications" className={isActive('/notifications') ? 'nav-link-active' : 'nav-link'}>
          <Bell size={15} />
          <span>Notifications</span>
        </Link>
        <Link href="/profile" className={isActive('/profile') ? 'nav-link-active' : 'nav-link'}>
          <User size={15} />
          <span>Profile</span>
        </Link>
        ...
        <button
          ...
          className="nav-link w-full text-danger/70 hover:text-danger hover:bg-danger/10"
        >
          <LogOut size={15} />
          <span>Sign out</span>
        </button>
